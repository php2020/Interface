...
local BCS_Tooltip = getglobal("BetterCharacterStatsTooltip") or
...
function BCS:GetHitRating(hitOnly)
	...
	_, _, hitFromAura = BCS:GetPlayerAura(L["Chance to hit decreased by (%d+)%% and %d+ Nature damage every %d+ sec."],
		'HARMFUL')
	if hitFromAura then
		hit_debuff = hit_debuff + tonumber(hitFromAura)
	end
	...
	if Cache_GetHitRating_Tab and Cache_GetHitRating_Talent then
		...
		for line = 1, MAX_LINES do
			...
			if left:GetText() then
				-- rogues
				local _, _, value = strfind(left:GetText(),
					L["Increases your chance to hit with melee weapons by (%d)%%."])
				local name, iconTexture, tier, column, rank, maxRank, isExceptional, meetsPrereq = GetTalentInfo(
				Cache_GetHitRating_Tab, Cache_GetHitRating_Talent)
				...
				-- hunters
				_, _, value = strfind(left:GetText(),
					L
					["Increases hit chance by (%d)%% and increases the chance movement impairing effects will be resisted by an additional %d+%%."])
				name, iconTexture, tier, column, rank, maxRank, isExceptional, meetsPrereq = GetTalentInfo(
				Cache_GetHitRating_Tab, Cache_GetHitRating_Talent)
				if value and rank > 0 then
					hit = hit + tonumber(value)
					line = MAX_LINES
		...
		for talent = 1, MAX_TALENTS do
			...
			for line = 1, MAX_LINES do
				...
				if left:GetText() then
					...
					local _, _, value = strfind(left:GetText(),
						L["Increases your chance to hit with melee weapons by (%d)%%."])
					local name, iconTexture, tier, column, rank, maxRank, isExceptional, meetsPrereq = GetTalentInfo(tab,
						talent)
					if value and rank > 0 then
						hit = hit + tonumber(value)
					...
					_, _, value = strfind(left:GetText(),
						L
						["Increases hit chance by (%d)%% and increases the chance movement impairing effects will be resisted by an additional %d+%%."])
					name, iconTexture, tier, column, rank, maxRank, isExceptional, meetsPrereq = GetTalentInfo(tab,
						talent)
					if value and rank > 0 then
						hit = hit + tonumber(value)
	...
	for slot = 0, MAX_INVENTORY_SLOTS do
		...
		if hasItem then
			...
			for line = 1, MAX_LINES do
				...
				if left:GetText() then
					local _, _, value = strfind(left:GetText(),
						L["Equip: Improves your chance to hit with spells by (%d)%%."])
					if value then
						hit = hit + tonumber(value)
					end
		...
		for talent = 1, MAX_TALENTS do
			...
			for line = 1, MAX_LINES do
				...
				if left:GetText() then
					...
					-- Elemental Precision
					local _, _, value = strfind(left:GetText(),
						L["Reduces the chance that the opponent can resist your Frost and Fire spells by (%d)%%."])
					local name, iconTexture, tier, column, rank, maxRank, isExceptional, meetsPrereq = GetTalentInfo(tab,
						talent)
					if value and rank > 0 then
					...
					-- Arcane Focus
					_, _, value = strfind(left:GetText(),
						L["Reduces the chance that the opponent can resist your Arcane spells by (%d+)%%."])
					local name, iconTexture, tier, column, rank, maxRank, isExceptional, meetsPrereq = GetTalentInfo(tab,
						talent)
					if value and rank > 0 then
					...
					-- Shadow Focus
					_, _, value = strfind(left:GetText(),
						L["Reduces your target's chance to resist your Shadow spells by (%d+)%%."])
					local name, iconTexture, tier, column, rank, maxRank, isExceptional, meetsPrereq = GetTalentInfo(tab,
						talent)
					if value and rank > 0 then
...
function BCS:GetCritChance()
	...
	if class == 'HUNTER' then
		...
		if Cache_GetCritChance_Tab and Cache_GetCritChance_Talent then
			...
			for line = 1, MAX_LINES do
				...
				if left:GetText() then
					local _, _, value = strfind(left:GetText(),
						L["Increases your critical strike chance with all attacks by (%d)%%."])
					local name, iconTexture, tier, column, rank, maxRank, isExceptional, meetsPrereq = GetTalentInfo(
					Cache_GetCritChance_Tab, Cache_GetCritChance_Talent)
					if value and rank > 0 then
	...
	if Cache_GetCritChance_SpellID and Cache_GetCritChance_BookType and Cache_GetCritChance_Line then
		...
		if left:GetText() then
			local value = string.gsub(left:GetText(), "(.*)%%.*", "%1")
	...
	-- end
	--修改by狗血编剧男
	local id ,BOOKTYPE_SPELL= zUI.api.libspell.GetSpellIndex(zUI.ATTACK)
	BCS_Tooltip:SetSpell(id, BOOKTYPE_SPELL)
	local left2 = getglobal(BCS_Prefix .. "TextLeft" .. 2):GetText()
	local iCritInfo = string.gsub(left2, "(.*)%%.*", "%1")
	if(iCritInfo) then
...
function BCS:GetRangedCritChance()
	...
	if Cache_GetRangedCritChance_Tab and Cache_GetRangedCritChance_Talent and Cache_GetRangedCritChance_Line then
		...
		if left:GetText() then
			local _, _, value = strfind(left:GetText(),
				L["Increases your critical strike chance with ranged weapons by (%d)%%."])
			local name, iconTexture, tier, column, rank, maxRank, isExceptional, meetsPrereq = GetTalentInfo(
			Cache_GetRangedCritChance_Tab, Cache_GetRangedCritChance_Talent)
			if value and rank > 0 then
				crit = crit + tonumber(value)
			end
		end
...
function BCS:GetSpellCritChance()
	...
	for slot = 0, MAX_INVENTORY_SLOTS do
		...
		if hasItem then
			...
			for line = 1, BCS_Tooltip:NumLines() do
				...
				if left:GetText() then
					local _, _, value = strfind(left:GetText(),
						L["Equip: Improves your chance to get a critical strike with spells by (%d)%%."])
					if value then
					...
					_, _, value = strfind(left:GetText(),
						L["^Set: Improves your chance to get a critical strike with spells by (%d)%%."])
	...
	_, _, critFromAura = BCS:GetPlayerAura(L
	["While active, target's critical hit chance with spells and attacks increases by 10%%."])
	if critFromAura then
		spellCrit = spellCrit + 10
	end
	_, _, critFromAura = BCS:GetPlayerAura(L
	["Increases chance for a melee, ranged, or spell critical by (%d+)%% and all attributes by %d+."])
	if critFromAura then
		spellCrit = spellCrit + tonumber(critFromAura)
	end
	critFromAura = BCS:GetPlayerAura(L
	["Increases critical chance of spells by 10%%, melee and ranged by 5%% and grants 140 attack power. 120 minute duration."])
	if critFromAura then
		spellCrit = spellCrit + 10
	end
	_, _, critFromAura = BCS:GetPlayerAura(L
	["Critical strike chance with spells and melee attacks increased by (%d+)%%."])
	if critFromAura then
		spellCrit = spellCrit + tonumber(critFromAura)
	end
	-- debuffs
	_, _, _, critFromAura = BCS:GetPlayerAura(
	L["Melee critical-hit chance reduced by (%d+)%%.\r\nSpell critical-hit chance reduced by (%d+)%%."], 'HARMFUL')
	if critFromAura then
		spellCrit = spellCrit - tonumber(critFromAura)
	end
		...
		for slot = 0, MAX_INVENTORY_SLOTS do
			...
			if hasItem then
				for line = 1, BCS_Tooltip:NumLines() do
					...
					if left:GetText() then
						local _, _, value = strfind(left:GetText(),
							L["Equip: Increases damage done by " .. school .. " spells and effects by up to (%d+)."])
						if value then
		...
		for slot = 0, MAX_INVENTORY_SLOTS do
			...
			if hasItem then
				...
				for line = 1, BCS_Tooltip:NumLines() do
					...
					if left:GetText() then
						local _, _, value = strfind(left:GetText(),
							L["Equip: Increases damage and healing done by magical spells and effects by up to (%d+)."])
						if value then
							spellPower = spellPower + tonumber(value)
						end
						...
						_, _, value = strfind(left:GetText(),
							L["Equip: Increases damage done by Arcane spells and effects by up to (%d+)."])
						if value then
						...
						_, _, value = strfind(left:GetText(),
							L["Equip: Increases damage done by Fire spells and effects by up to (%d+)."])
						if value then
						...
						_, _, value = strfind(left:GetText(),
							L["Equip: Increases damage done by Frost spells and effects by up to (%d+)."])
						if value then
						...
						_, _, value = strfind(left:GetText(),
							L["Equip: Increases damage done by Holy spells and effects by up to (%d+)."])
						if value then
						...
						_, _, value = strfind(left:GetText(),
							L["Equip: Increases damage done by Nature spells and effects by up to (%d+)."])
						if value then
						...
						_, _, value = strfind(left:GetText(),
							L["Equip: Increases damage done by Shadow spells and effects by up to (%d+)."])
						if value then
						...
						_, _, value = strfind(left:GetText(),
							L["^Set: Increases damage and healing done by magical spells and effects by up to (%d+)%."])
			...
			for talent = 1, MAX_TALENTS do
				...
				for line = 1, MAX_LINES do
					...
					if left:GetText() then
						...
						-- Spiritual Guidance
						local _, _, value = strfind(left:GetText(),
							L["Increases spell damage and healing by up to (%d+)%% of your total Spirit."])
						local name, iconTexture, tier, column, rank, maxRank, isExceptional, meetsPrereq = GetTalentInfo(
						tab, talent)
						if value and rank > 0 then
...
function BCS:GetHealingPower()
	...
	for slot = 0, MAX_INVENTORY_SLOTS do
		...
		if hasItem then
			...
			for line = 1, BCS_Tooltip:NumLines() do
				...
				if left:GetText() then
					local _, _, value = strfind(left:GetText(),
						L["Equip: Increases healing done by spells and effects by up to (%d+)."])
					if value then
					...
					_, _, value = strfind(left:GetText(),
						L["^Set: Increases healing done by spells and effects by up to (%d+)%."])
	...
	--Sweet Surprise
	_, _, healPowerFromAura = BCS:GetPlayerAura(L
	["Increases healing done by magical spells by up to (%d+) for 3600 sec."])
	if healPowerFromAura then
		healPower = healPower + tonumber(healPowerFromAura)
	end
	--Unstable Power
...
function BCS:GetManaRegen()
		...
		if hasItem then
			...
			for line = 1, BCS_Tooltip:NumLines() do
				...
				if left:GetText() then
					...
					_, _, value = strfind(left:GetText(),
						"^Equip: Allows (%d+)%% of your Mana regeneration to continue while casting.")
					if value then
					...
					_, _, value = strfind(left:GetText(),
						L["^Set: Allows (%d+)%% of your Mana regeneration to continue while casting."])
		...
		for talent = 1, MAX_TALENTS do
			...
			for line = 1, MAX_LINES do
				...
				if left:GetText() then
					...
					local _, _, value = strfind(left:GetText(),
						L["Allows (%d+)%% of your Mana regeneration to continue while casting."])
					local name, iconTexture, tier, column, rank, maxRank, isExceptional, meetsPrereq = GetTalentInfo(tab,
						talent)
					if value and rank > 0 then
						casting = casting + tonumber(value)
	...
	-- buffs
	-- Warchief's Blessing
	local _, _, mp5FromAura = BCS:GetPlayerAura(L
	["Increases hitpoints by 300. 15%% haste to melee attacks. (%d+) mana regen every 5 seconds."])
	if mp5FromAura then
		mp5 = mp5 + 10
	end
	--Epiphany
