...
zUI:RegisterComponent("zChat", function()
    ...
    local chatevents                      = {
        ...
        CHAT_MSG_SYSTEM = {
            ...
            ['(.+) has reached level (.+) in Hardcore mode! Their ascendance towards immortality continues, however, so do the dangers they will face.'] = '|cfff86256[HC]|r %1克服万难成功晋升到%2级,飞升指日可待！',
            ...
            ['(.+) has Laughed in the face of death in the Hardcore challenge. (.+) has begun the Inferno Challenge!'] = '%1开启了硬核挑战模式,微笑着直面地狱般的挑战！',
            ['Server Time: (.+)']                                                                                                                        = '服务器时间：%1',
            ...
            ['You have gained (.+) hit points and (.+) mana.']                                                                                           = 'HP +%1 MP +%2',
            ...
            ['(.+) was added to your collection.'] = "%1 添加到收藏",
            ['You have gained (.+) hit points.']='HP +%1'
        },
    ...
    local AddMessage = function(f, t, r, g, b, id)
        ...
        if (zUI_config.chat.tformat == "1") then
            ...
            if C.quality.off_chattime ~='1' then
