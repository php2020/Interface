...
function zUI.ToggleSelfCast()
	...
	if C.bars.keydown == "1" then
		...
		zUI.api.hooksecurefunc("ActionButtonDown", function(id)	-- todo get an option for this!!! and all other colors..
			ActionButtonUp(id)
			if ( BonusActionBarFrame:IsShown() ) then
				local button = getglobal("BonusActionButton"..id);
		...
		zUI.api.hooksecurefunc("ActionButtonUp", function(id)
			if ( BonusActionBarFrame:IsShown() ) then
				local button = getglobal("BonusActionButton"..id);
		...
		zUI.api.hooksecurefunc("MultiActionButtonDown", function(bar, id)
		...
		zUI.api.hooksecurefunc("MultiActionButtonUp", function(bar, id, onSelf)
			local button = getglobal(bar.."Button"..id);
			--zSkinColor(button, .2, .2, .2);
			zSkinColor(button, cr, cg, cb); 
			button:SetChecked(0);
		...
		--------------==[ Pet ]==---------------------------------------
		zUI.api.hooksecurefunc("PetActionButtonDown", function(id) -- dont need final say with these functions I think. wrong?!
		...
		zUI.api.hooksecurefunc("PetActionButtonUp", function(id)
			local button = getglobal("PetActionButton"..id);
