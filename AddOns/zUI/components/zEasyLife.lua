...
zUI:RegisterComponent("[内核]基础功能", function ()
	...
	if (C.quality.auto_stance == "1") then
		...
		zUI.api.hooksecurefunc("CastSpell", function(spellId, spellbookTabNum) -- todo: add option for Auto-Switch Stance.
			zUI.zEasyLife:SwitchStance()
		end)

		zUI.api.hooksecurefunc("CastSpellByName", function(spellName, onSelf)
			zUI.zEasyLife:SwitchStance()
		end)

		zUI.api.hooksecurefunc("UseAction", function(slot, checkCursor, onSelf)
			zUI.zEasyLife:SwitchStance()
		end)
		zUI.api.hooksecurefunc("cast", function(slot, checkCursor, onSelf)
			zUI.zEasyLife:SwitchStance()
		end)
	end
	...
	"spell_nature_sentinal",
		...
		zUI.zEasyLife:SetScript("OnEvent", function()
			...
			--mrbcat20230813 屏蔽错误
			for _,err in pairs(zUI.zEasyLife.errarry) do 
				if string.find(arg1, err, 1) then
					...
					for i=0,31,1 do
						...
						if (currBuffTex) then
							...
							for id, bufftype in pairs(zUI.zEasyLife.buffs) do
								if string.find(string.lower(currBuffTex), string.lower(bufftype), 1) then
