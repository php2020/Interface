...
zUI:RegisterComponent("zLoot", function ()
	...
	function zUI.loot.RaidRoll(candidates)
		...
		if to >= 1 then
			...
			if quality >= tonumber(C.loot.rollannouncequal) then
				zUI.api.SendChatMessageWide(T["Random Rolling"].." "..GetLootSlotLink(slot))
				if C.loot.rollannounce == "1" then
					...
					for i=1,to do
						...
						if i == to or k == 15 then
							zUI.api.QueueFunction(zUI.api.SendChatMessageWide,names)
							names = ""
						end
						k = k<15 and k+1 or 1
					end
			...
			zUI.api.QueueFunction(RandomRoll,"1",tostring(to))
		end
	end
	...
	function zUI.loot.RequestRolls()
		...
		zUI.api.SendChatMessageWide(T["Roll for"].. " " .. GetLootSlotLink(slot))
		zUI.loot:RegisterEvent("CHAT_MSG_SYSTEM")
	...
	function zUI.loot.CallTieRoll(rollers)
		...
		if num_ties > 1 then
			...
			zUI.loot:ClearRolls()
			zUI.api.SendChatMessageWide(names.." "..T["Reroll"])
			zUI.loot:RegisterEvent("CHAT_MSG_SYSTEM")
			zUI.loot.monitorRolling = true
		end
		UIDropDownMenu_Refresh(GroupLootDropDown)
	...
	function zUI.loot:BuildRaidMenu(level)
		...
		for i = 1, MAX_RAID_MEMBERS do -- masterlootcandidate index does not correspond with unit id index
			...
			if (candidate) then
				...
				local unit = zUI.api.GroupInfoByName(candidate,"raid")
	...
	function zUI.loot:BuildPartyMenu(level)
		...
		if level == 1 then
			for i=1, MAX_PARTY_MEMBERS+1, 1 do
				...
				if (candidate) then
					info = wipe(info)
					local unit = zUI.api.GroupInfoByName(candidate,"party")
					info.text = candidate
	...
	if C.loot.advancedloot == "1" then
		...
		zUI.api.hooksecurefunc("UnitPopup_HideButtons",function()
			local dropdownFrame = _G[UIDROPDOWNMENU_INIT_MENU]
			local unit = dropdownFrame.unit
			local name = dropdownFrame.name
...
--默认是26
			zUI.loot:SetHeight(math.max((real*lootsize)+2*C.appearance.border.default), 20)
			zUI.loot:SetWidth(maxwidth + lootsize + 8*C.appearance.border.default )
		end
	end
	...
	function zUI.loot:CreateSlot(id)
		...
		--frame:SetHeight(22)
		--修改拾取框体内图标大小
		frame:SetHeight(lootsize-6)
		--frame:SetPoint("TOP", zUI.loot, "TOP", 4, (-C.appearance.border.default*2+22)-(id*22))
		frame:SetPoint("TOP", zUI.loot, "TOP", 4, (-C.appearance.border.default*2+lootsize)-(id*lootsize))
		...
		frame.count:SetFont(STANDARD_TEXT_FONT, C.global.font_size+2, "OUTLINE")
		...
		frame.name:SetFont(STANDARD_TEXT_FONT, lootfontsize, "OUTLINE")
	...
	zUI.loot:SetScript("OnEvent", function()
		...
		if event == "CHAT_MSG_SYSTEM" then
			...
			if zUI.loot.monitorRolling ~= nil then
				...
				zUI.api.QueueFunction(UIDropDownMenu_Refresh,GroupLootDropDown)
			end
		end

		if C.loot.autopickup == "1" and event == "LOOT_BIND_CONFIRM" then
			zUI.api.QueueFunction(AutoBind,arg1)
		end
	end)
