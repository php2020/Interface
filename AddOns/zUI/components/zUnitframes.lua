zUI:RegisterComponent("zUnitframes", function()
	...
	function zUI.zUnitframes.Style_PlayerFrame()
		if (C.unitframes.compactmode == "0") then
			...
			PlayerFrameHealthBar:SetPoint("TOPLEFT", 106, -22);
			--mrbcat-20230730微调血量位置
			PlayerFrameHealthBarText:SetPoint("CENTER", 50, 18);
	...
	function UnitFramesImproved_ClassPortraits()
		if (C.unitframes.classportraits == "1") then
			...
			zUI.api.hooksecurefunc("SetPortraitTexture", function()
	...
	zUI.api.hooksecurefunc("HealthBar_OnValueChanged", zHealthBar_OnValueChanged, true); -- can be after orig..
	...
	zUI.api.hooksecurefunc("TargetFrame_CheckFaction", zTargetFrame_OnUpdate, true); --run after default.
	zUI.api.hooksecurefunc("TargetFrame_CheckClassification", zTargetFrame_CheckClassification, true);
	zUI.api.hooksecurefunc("PetFrame_OnUpdate", zPetFrame_OnUpdate, true);          --run after default.
	zUI.api.hooksecurefunc("TextStatusBar_UpdateTextString", zTextStatusBar_UpdateTextString, true);
...
--mrbcat-20230729微调状态值位置
	if (C.unitframes.compactmode == "1") then
		zTargetHealthText:SetPoint('CENTER', TargetFrameHealthBar, 0, 0) -- -2
		zTargetPowerText:SetPoint('CENTER', TargetFrameManaBar, 0, 1) -- 1
	else
	...
	function zUI.MobHealth:HealthUpdate()
		...
		if uBoth == 1 then
			if max == 100 then
				...
				if (C.unitframes.percentages == "1") then
					--mrbcat20230731 修改生命值显示格式 当前 + 百分比
	...
	zUI.api.hooksecurefunc("TargetHealthCheck", function()
	...
	--- added,
	zUI.api.hooksecurefunc("PlayerFrame_UpdatePvPStatus", function() --todo move to correct place.
	...
	zUI.api.hooksecurefunc("TargetFrame_CheckFaction", function()
