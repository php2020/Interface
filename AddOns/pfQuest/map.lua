  ...
  if meta["quest"] then
    ...
    for qid=1, GetNumQuestLogEntries() do
      ...
      if meta["quest"] == qtitle then
        ...
        --mrbcat-20230811
        tooltip:AddLine(symbol .. (pfDB.CatQname[meta["quest"]] or meta["quest"]), 1, 1, 0)
            ...
            elseif table.getn(meta["item"]) > 0 and type == "item" and meta["droprate"] then
              ...
              for mid, item in pairs(meta["item"]) do
                if item == itemName then
                  ...
                  -- local cIname = (pfDB.CatIname[item] or pfDB.CatUname[item] or item)
                  -- zPrint(cIname.."droprate" or "kongkong ")
    ...
    if not catch then
      tooltip:AddLine("|cff555555[|cffffcc00!|cff555555]|r " .. (pfDB.CatQname[meta["quest"]] or meta["quest"]), 1, 1, .7)
    end
