...
local function ShowTooltip()
  if this.tooltip then
    ...
    if this.text then
      --zPrint(this.text:GetText() or "内容为空")
      GameTooltip:SetText(this.text:GetText())--标题带进度
    ...
    if this.node and this.node.questid then
      if pfDB["quests"] and pfDB["quests"]["loc"] and pfDB["quests"]["loc"][this.node.questid] and pfDB["quests"]["loc"][this.node.questid]["O"] then
        --mrbcat 任务追踪 鼠标滑过左下角提示
        GameTooltip:AddLine(pfDatabase:FormatQuestText(pfDB["quests"]["zhCN"][this.node.questid]["O"]), 1,1,1,1)
        GameTooltip:AddLine(" ")
      end
...
function tracker.ButtonEvent(self)
  ...
  local title  = pfDB.CatQname[self.title] or self.title
  ...
  if tracker.mode == "QUEST_TRACKING" then
    ...
    local objectives = GetNumQuestLeaderBoards(qlogid) --任务需求项 2项
