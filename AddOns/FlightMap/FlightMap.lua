...
local lTYPE_HORDE     = FLIGHTMAP_HORDE; -- 部落
local lTYPE_ALLIANCE  = FLIGHTMAP_ALLIANCE; -- 联盟
local lTYPE_CONTESTED = FLIGHTMAP_CONTESTED; -- 争夺中
...
-- 地区的颜色
FLIGHTMAP_COLORS = {
    Unknown   = { r = 0.8, g = 0.8, b = 0.8 }, -- 未知
    Hostile   = { r = 0.9, g = 0.2, b = 0.2 }, -- 敌对
    Friendly  = { r = 0.2, g = 0.9, b = 0.2 }, -- 友善
    Contested = { r = 0.8, g = 0.6, b = 0.4 }, -- 争夺
};
...
local function lSetDefaultData()
    ...
    -- Patch 1.8: 删除对 Valor‘s Rest 的任何引用
    lStripPoint(FlightMap[FLIGHTMAP_HORDE] or {}, "1:461:226"); -- 部落
    lStripPoint(FlightMap[FLIGHTMAP_ALLIANCE] or {}, "1:463:223"); -- 联盟

    -- Patch 1.12: 删除对联盟错位Moonglade的任何引用
    lStripPoint(FlightMap[FLIGHTMAP_ALLIANCE] or {}, "1:552:793"); -- 联盟
...
local function lLearnTaxiNode()
    ...
    for index = 1, numNodes, 1 do
        ...
        elseif (tType == "REACHABLE") then
            local mx, my = TaxiNodePosition(index); -- 可达
    ...
    if (thisNode) then
        ...
        for k,v in pairs(destinations) do
            ...
            if routes > 1 then
                ...
                local newRoute = {};
                for r = 1, routes do
                    local dest = FlightMapUtil.makeNodeName(thisCont,TaxiGetDestX(v, r), TaxiGetDestY(v, r));
...
local function lAddFlightsForNode(tooltip, node, prefix, source)
    ...
    if data.Location.Zone then
        locn = string.format("%d, %d", data.Location.Zone.x * 100,data.Location.Zone.y * 100);
    end
...
function FlightMap_OnLoad()
    ...
    if (Sea) then
        Sea.util.hook("WorldMapButton_OnUpdate", "FlightMap_WorldMapButton_OnUpdate","after");
    else
