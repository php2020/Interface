...
QuestStatusData = {};
QuestStatusData[QUESTSTATUS_ACTIVE] = {
    Red = 0,
    Green = 1,
    Blue = 0,
    StatusText = QUESTITEM_QUESTACTIVE
};
QuestStatusData[QUESTSTATUS_COMPLETEABANDONED] = {
    Red = 0.7,
    Green = 0.2,
    Blue = 0.5,
    StatusText = QUESTITEM_COMPLETEABANDONED
};
QuestStatusData[QUESTSTATUS_ABANDONED] = {
    Red = 0.7,
    Green = 0,
    Blue = 0,
    StatusText = QUESTITEM_ABANDONED
};
QuestStatusData[QUESTSTATUS_COMPLETE] = {
    Red = 0.7,
    Green = 0.7,
    Blue = 0.7,
    StatusText = QUESTITEM_QUESTCOMPLETE
};
...
function QuestItem_UpdateItem(item, quest, count, total, status)
    ...
    if (QuestItem_SearchString(
        QuestItems[item].QuestName, QUESTITEM_UNIDENTIFIED) and
        not QuestItem_SearchString(quest, QuestItems[item].QuestName)) then
        QuestItems[item].QuestName = quest;
    end
    ...
    if (count ~= nil and QuestItem_CheckNumeric(count)) then
        QuestItems[item][UnitName("player")].Count =
            QuestItem_MakeIntFromHexString(
                count);
    else
...
function QuestItem_FindQuest(item, searchQuestHistory)
    ...
    for y = 1, GetNumQuestLogEntries(), 1 do
        local qName, level, questTag, isHeader, isCollapsed, complete =
            GetQuestLogTitle(
                y);
        QuestName = qName;
        -- Don't check headers
        if (not isHeader) then
            ...
            if (GetNumQuestLeaderBoards() > 0) then
                ...
                for i = 1, GetNumQuestLeaderBoards(), 1 do -- Objectives
                    ...
                    if (itemType ~= nil and
                        (itemType == "item" or itemType == "object")) then
...
function QuestItem_FindQuestInQuestHistory(questItemName)
    ...
    if (QuestItems[item]) then
        if (QuestItems[item][UnitName("player")] and
            QuestItems[item][UnitName("player")].QuestStatus ~=
            QUESTSTATUS_ACTIVE) then
            status = QuestItems[item][UnitName("player")].QuestStatus;
        end
        QuestName = QuestItems[item].QuestName;
    end
                ...
                for questIndex, questValue in charValue do
                    ...
                    if (questValue["os"]) then
                        for objectiveIndex, objective in questValue["os"] do
                            ...
                            if (itemName and
                                QuestItem_SearchString(itemName, questItemName)) then
                                -- Get information on the item
                                local itemText, count, total =
                                    QuestItem_GetItemInfo(
                                        itemName);
...
function QuestItem_LocateQuest(itemText, itemCount, itemTotal)
    ...
    if (not QuestItems[itemText] or QuestItems[itemText].QuestName ==
        QUESTITEM_UNIDENTIFIED) then
...
function QuestItem_SetQuestStatus(questTitle, questStatus)
    if (questTitle) then
        ...
        questTitle = string.gsub(
                         string.gsub(questTitle, '^[[].*[]]', ''), '^ ', '');
        for itemName, itemData in QuestItems do
            if (itemData["QuestName"] and itemData.QuestName == questTitle) then
                QuestItem_UpdateItem(
                    itemName, questTitle, 0, QuestItems[itemName].Total,
                        questStatus);
...
function QuestItem_OnEvent(event)
    ...
    elseif (event == "QUEST_COMPLETE") then
        QuestItem_Debug(
            "Completing quest " .. GetQuestLogTitle(GetQuestLogSelection()));
    end
end
...
function QuestItem_VariablesLoaded()
    if (QuestItem_Settings and QuestItem_Settings["version"] ==
        QUESTITEM_VERSION) then
        return;
    end
...
function QuestItem_Enabled()
    if (QuestItem_Settings["Enabled"] == nil or QuestItem_Settings["Enabled"] ==
        false) then
        return false;
    end
    return true;
    ...
    if (DEBUG) then
        if (not message) then
            DEFAULT_CHAT_FRAME:AddMessage(
                "Debug: Message was nil", 0.9, 0.5, 0.3);
        else
...
function QuestItem_OpenQuestLog(questName)
    for y = 1, GetNumQuestLogEntries(), 1 do
        local QuestName, level, questTag, isHeader, isCollapsed, complete =
            GetQuestLogTitle(
                y);
