		...
		if(((event == "LOOT_OPENED") or (pre == "LOOT_OPENED")) and (player)) then			
			for i = 1, GetNumRaidMembers(),1 do
				...
				if ((name == player) and targetname and targetdead and boss) then 					
					if ((GetTime() - iloottime > 120) or (ilootcount < 3)) then	
						if(GetTime() - iloottime > 120) then
							SendChatMessage("黑手===>>> ["..UnitName("raid"..i).."]".."第一个打开了 "..targetname.." 的尸体","RAID")	
							SendChatMessage("黑手===>>> ["..UnitName("raid"..i).."]".."第一个打开了 "..targetname.." 的尸体","YELL")
							iloottime = GetTime()
							ilootcount = 0
						end							
