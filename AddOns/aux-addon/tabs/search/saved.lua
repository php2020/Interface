	...
	OnClick = function(st, data, _, button)
		...
		elseif button == 'RightButton' then
			...
			elseif st == favorite_searches_listing then
				...
				gui.menu(
					(auto_buy and G_DISABLE_M  or G_ENABLE_M ) .. G_AUTO_BUY_M, function() if auto_buy then data.search.auto_buy = nil else enable_auto_buy(data.search) end u() end,
					(auto_bid and G_DISABLE_M  or G_ENABLE_M ) .. G_AUTO_BIG_M, function() if auto_bid then data.search.auto_bid = nil else enable_auto_bid(data.search) end u() end,
					G_MOVE_UP_M, function() move_up(favorite_searches, data.index); u() end,
					G_MOVE_DOWN_M, function() move_down(favorite_searches, data.index); u() end,
					G_DELETE_M, function() tremove(favorite_searches, data.index); u() end
				)
			end
		end
	end,
	...
	for _, search in favorite_searches do
		if search.auto_buy then
			...
			if queries then
				...
				aux.print(G_INVALID_AUTO_BUY_FILTER, error)
			end
		end
	end
	...
	for _, search in favorite_searches do
		if search.auto_bid then
			...
			if queries then
				...
				aux.print(G_INVALID_AUTO_BID_FILTER, error)
			end
		end
	end
	return function(record)
...
function add_favorite(filter_string)
	...
	if queries then
		...
		aux.print(G_INVALID_FILTER, error)
	end
end
...
function enable_auto_buy(search)
	...
	if queries then
		if getn(queries) > 1 then
			aux.print(G_ERROR_AUTO_BUY_DOES_NOT_SUPPORT_MULTI_QUERIES)
		elseif aux.size(queries[1].blizzard_query) > 0 and not filter_util.parse_filter_string(search.filter_string).blizzard.exact then
			aux.print(G_ERROR_AUTO_BUY_DOES_NOT_SUPPORT_BLIZZARD_FILTERS)
		else
...
function enable_auto_bid(search)
	...
	if queries then
		if getn(queries) > 1 then
			aux.print(G_ERROR_AUTO_BID_DOES_NOT_SUPPORT_MULTI_QUERIES)
		elseif aux.size(queries[1].blizzard_query) > 0 and not filter_util.parse_filter_string(search.filter_string).blizzard.exact then
			aux.print(G_ERROR_AUTO_BID_DOES_NOT_SUPPORT_BLIZZARD_FILTERS)
		else
