...
function start_real_time_scan(query, search, continuation)
	...
	search_scan_id = scan.start{
		...
		on_scan_start = function()
			...
			search.status_bar:set_text(G_SCANNING_LAST_PAGE)
		end,
		...
		on_abort = function()
			search.status_bar:update_status(1, 1)
			search.status_bar:set_text(G_SCAN_PAUSED)
...
function start_search(queries, continuation)
	...
	search_scan_id = scan.start{
		...
		on_scan_start = function()
			...
			if continuation then
				search.status_bar:set_text(G_RESUMING_SCAN)
			else
				search.status_bar:set_text(G_SCANNING_AUCTIONS)
			end
		end,
		on_page_loaded = function(_, total_scan_pages)
			...
			search.status_bar:set_text(format(G_SCANNING_D___D_PAGE_D___D, current_query, total_queries, current_page, total_scan_pages))
		end,
		...
		on_complete = function()
			search.status_bar:update_status(1, 1)
			search.status_bar:set_text(G_SCAN_COMPLETE)
...
function M.execute(resume, real_time)
	...
	elseif real_time then
		if getn(queries) > 1 then
			aux.print(G_ERROR_THE_REAL_TIME_MODE_DOES_NOT_SUPPORT_MULTI_QUERIES)
			return
		elseif queries[1].blizzard_query.first_page or queries[1].blizzard_query.last_page then
			aux.print(G_ERROR_THE_REAL_TIME_MODE_DOES_NOT_SUPPORT_PAGE_RANGES)
			return
		end
	end
