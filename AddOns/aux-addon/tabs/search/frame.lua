...
do
	...
	btn:SetHeight(25)
	btn:SetWidth(60)
	btn:SetText(aux.color.label.enabled(G_RANGE))
...
do
	...
	btn:SetWidth(60)
	btn:Hide()
	btn:SetText(aux.color.label.enabled(G_REAL_TIME))
	btn:SetScript('OnClick', function()
...
do
    ...
    scroll_frame:SetScript('OnMouseWheel', function()
	    ...
	    child:SetFont('p', [[Fonts\FZBWJW.TTF]], aux.bounded(gui.font_size.small, gui.font_size.large, aux.select(2, child:GetFont()) + arg1*2))
    ...
    scroll_child:SetFont('p', [[Fonts\FZBWJW.TTF]], gui.font_size.large)
...
favorite_searches_listing:SetColInfo{{name=G_AUTO, width=.07, align='CENTER'}, {name=G_FAVORITE_SEARCHES, width=.93}}

recent_searches_listing = listing.new(frame.saved.recent)
recent_searches_listing:SetColInfo{{name=G_RECENT_SEARCHES, width=1}}
