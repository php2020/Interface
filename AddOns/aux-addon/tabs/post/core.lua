...
function update_item_configuration()
	if not selected_item then
        ...
        item.name:SetText(G_NO_ITEM_SELECTED)
        ...
        do
            ...
            deposit:SetText(G_DEPOSIT .. money.to_string(amount, nil, nil, aux.color.text.enabled))
        end
...
function refresh_entries()
	if selected_item then
        ...
        status_bar:set_text(G_SCANNING_AUCTIONS)
		...
		scan_id = scan.start{
			...
			on_page_loaded = function(page, total_pages)
                ...
                status_bar:set_text(format(G_SCANNING_PAGE_D___D, page, total_pages))
			end,
			...
			on_abort = function()
                ...
                status_bar:set_text(G_SCAN_ABORTED)
			end,
			on_complete = function()
                ...
                refresh = true
                status_bar:update_status(1, 1)
                status_bar:set_text(G_SCAN_COMPLETE)
            end,
		}
	end
end
