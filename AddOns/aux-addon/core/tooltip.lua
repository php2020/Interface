...
local data = DbItems
local auxlocal = GetLocale()
local UNKNOWN = GRAY_FONT_COLOR_CODE .. 'NA' .. FONT_COLOR_CODE_CLOSE
...
function M.extend_tooltip(tooltip, link, quantity)
    ...
    if item_info then
        ...
        if getn(distribution) > 0 then
            if settings.disenchant_distribution then
                tooltip:AddLine(G_DISENCHANTS_INTO, aux.color.tooltip.disenchant.distribution())
            ...
            if settings.disenchant_value then
                ...
                tooltip:AddLine(G_DISENTCHANT .. (disenchant_value and money.to_string2(disenchant_value) or UNKNOWN), aux.color.tooltip.disenchant.value())
            end
        end
    end
    if settings.merchant_buy then
        ...
        if price then
            tooltip:AddLine(G_VENDOR_BUY .. (limited and '(limited): ' or ': ') .. money.to_string2(price * quantity), aux.color.tooltip.merchant())
        end
    end
    local en,cn,lv,sell
    --根据RDbItems数据库返回价格
    if  data and data[item_id] ~= nil then
        local s
        
        en,cn,sell = data[item_id][1],data[item_id][2],data[item_id][4]
        if auxlocal =="zhCN" then
            s= en
            else
                s = cn
        end
        tooltip:AddLine(s.."   ItemID:"..tostring(item_id))
    end
    if settings.merchant_sell then
        local price = info.merchant_info(item_id)

		if price == nil  and data and data[item_id] ~= nil then
			local charges = 1
        ...
        if price ~= 0  then
            ...
            tooltip:AddLine(G_VENDOR .. (price and money.to_string2(price * quantity) or UNKNOWN), aux.color.tooltip.merchant())
        end
    end
    ...
    if auctionable then
        if settings.value then
            tooltip:AddLine(G_VALUE_AH .. (value and money.to_string2(value * quantity) or UNKNOWN), aux.color.tooltip.value())
        end
        if settings.daily  then
            local market_value = history.market_value(item_key)
            tooltip:AddLine(G_TODAT .. (market_value and money.to_string2(market_value * quantity) .. ' (' .. gui.percentage_historical(aux.round(market_value / value * 100)) .. ')' or UNKNOWN), aux.color.tooltip.value())
        end
    end
