-- GLOBALS: MiniNameplatesHandleEvent, MiniNameplatesUpdate
local _G = getfenv(0)
local ADDON = {}
...
ADDON.Icons = {
  ["DRUID"] = "Interface\\AddOns\\MiniNameplates\\Class\\ClassIcon_Druid",
  ["HUNTER"] = "Interface\\AddOns\\MiniNameplates\\Class\\ClassIcon_Hunter",
  ["MAGE"] = "Interface\\AddOns\\MiniNameplates\\Class\\ClassIcon_Mage",
  ["PALADIN"] = "Interface\\AddOns\\MiniNameplates\\Class\\ClassIcon_Paladin",
  ["PRIEST"] = "Interface\\AddOns\\MiniNameplates\\Class\\ClassIcon_Priest",
  ["ROGUE"] = "Interface\\AddOns\\MiniNameplates\\Class\\ClassIcon_Rogue",
  ["SHAMAN"] = "Interface\\AddOns\\MiniNameplates\\Class\\ClassIcon_Shaman",
  ["WARLOCK"] = "Interface\\AddOns\\MiniNameplates\\Class\\ClassIcon_Warlock",
  ["WARRIOR"] = "Interface\\AddOns\\MiniNameplates\\Class\\ClassIcon_Warrior",
  ["TargetIcon"] = "Interface\\AddOns\\MiniNameplates\\Reticule"
}
...
function ADDON.isPet(name)
  return ADDON.PetsENG[name] or false
end
...
function ADDON.targetIndicatorShow(namePlate)
  ...
  if MiniNameplates.scanningPlayers then return end
...
function ADDON.MiniNameplates_OnUpdate(elapsed)
  MiniNameplates.ticker = MiniNameplates.ticker + elapsed
  if not (MiniNameplates.ticker > ADDON.genSettings.refreshRate) then return end  -- cap at 60fps by default
  MiniNameplates.ticker = 0
  ...
  for _, namePlate in ipairs(frames) do
    if ADDON.IsNamePlateFrame(namePlate) then
      ...
      if UnitName("target") == nil and string_find(name, "%s") == nil and string_len(name) <= 12 and ADDON.Targets[name] == nil then --Set Name text and save it in a list
        MiniNameplates.scanningPlayers = true
        ADDON.fillPlayerDB(name)
        ClearTarget()
        MiniNameplates.scanningPlayers = false
      end
...
function MiniNameplatesHandleEvent(event) --Handles wow events
  
  if event == "VARIABLES_LOADED" then
    local options = _G["MiniNameplatesOptions"]()
    -- Settings block
    ADDON.genSettings = MiniNameplatesDBPC.genSettings
    ADDON.raidicon = MiniNameplatesDBPC.raidicon
    ADDON.debufficon = MiniNameplatesDBPC.debufficon
    ADDON.classicon = MiniNameplatesDBPC.classicon
    ADDON.targetindicator = MiniNameplatesDBPC.targetindicator
    ADDON.nametext = MiniNameplatesDBPC.nametext
    ADDON.leveltext = MiniNameplatesDBPC.leveltext
    ADDON.VARIABLES_LOADED = true
    if ADDON.PLAYER_ENTERING_WORLD then
      ADDON.PLAYER_ENTERING_WORLD = nil
      MiniNameplatesHandleEvent("PLAYER_ENTERING_WORLD")
    end
  end
...
function MiniNameplatesUpdate(elapsed) --updates the frames
  if not ADDON.VARIABLES_LOADED then return end
  ADDON.MiniNameplates_OnUpdate(elapsed)
end
