...
BINDING_NAME_SUPERINSPECT = "观察目标";
...
function SuperInspectFrame_Load_OnEvent()
	if( event == "VARIABLES_LOADED" ) then
		if( DEFAULT_CHAT_FRAME ) then
			DEFAULT_CHAT_FRAME:AddMessage("|cffffd200SuperInspect |cffffff00v"..SUPERINSPECT_VERSION.."已加载.");
...
function SuperInspect_SlashCommand(msg)
	if( not msg or msg == "" ) then
		DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20SuperInspect 命令:");
		DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20/superinspect scale <数字> |cff3366ff- 在0.25与2.0比例之间缩放观察窗口 (默认为0.75)");
		DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20/superinspect reset |cff3366ff- 将观察窗口重新放置在屏幕的正中");
		DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20/superinspect defaulttoggle |cff3366ff- 打开/关闭默认观察模式(打开此选项时, SI将与WOW默认观察窗口保持一样位置和大小. 关闭此选项时, 你可以拖动并调整观察窗口的大小)");
		DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20/superinspect itembgtoggle |cff3366ff- 切换物品质量的显示模式");
		DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20/superinspect durabilitytoggle |cff3366ff- 当观察自己时,显示/隐藏装备耐久度信息");
		DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20/superinspect sound |cff3366ff- 窗口显示和关闭时 打开/关闭 声音提示");
		DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20/superinspect playercache |cff3366ff- 启用或禁用玩家缓存");
		return;
	end
	if (string.find(msg, "scale") ~= nil) then
		...
		if (SuperInspect.s) then
			...
			DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20SuperInspect: SuperInspect观察窗口缩放比例设置为:"..SuperInspect.s);
		else
			DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20SuperInspect: 请在 'scale' 后输入要缩放的比例(合法的缩放比例: 0.5-2.0, 例如: /superinspect scale 0.8)");
		end
		return;
--/script CSG_Main_Frame:SetScale(UIParent:GetScale() * 0.75);
	...
	if( msg == "reset" ) then
		...
		SuperInspect_ResetFrame();
		DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20SuperInspect: 已将观察窗口重新放置在屏幕的正中");
	end
	...
	if( msg == "defaulttoggle" ) then
		...
		if (SI_Save.default) then
			SI_Save.default = nil;
			SuperInspect_ResetFrame();
			DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20SuperInspect: 暴雪默认模式已关闭 (可以移动和缩放)");						
		else
			SI_Save.default = 1;
			SuperInspect_ResetFrame();
			DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20SuperInspect: 暴雪默认模式已开启 (大小和位置已锁定)");			
		end
	end
	if( msg == "itembgtoggle" ) then
		...
		elseif (SI_Save.itembg == 1) then
			...
			DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20SuperInspect: 关闭物品质量颜色显示");						
		else
			SI_Save.itembg = nil;
			if (SuperInspectFrame:IsVisible()) then
				SuperInspectFrame:Hide();
	...
	if( msg == "durabilitytoggle" ) then
		...
		if (SI_Save.durabilityoff) then
			...
			SuperInspect_ResetFrame();
			DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20SuperInspect: 显示耐久度信息");						
		else
			SI_Save.durabilityoff = 1;
			SuperInspect_ResetFrame();
			DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20SuperInspect: 隐藏耐久度信息");	
		end
	end
	if( msg == "sound" ) then
		if (not UIParentLoadAddOn("SuperInspect_UI")) then
		...
		if (SI_Save.snd) then
			SI_Save.snd = nil;
			DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20SuperInspect: 关闭声音提示");						
		else
			SI_Save.snd = 1;
			PlaySound("igCharacterInfoOpen");
			DEFAULT_CHAT_FRAME:AddMessage("|cff20ff20SuperInspect: 打开声音提示");	
		end
	end
end
